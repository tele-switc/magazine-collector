...
MAGAZINES = {
    "economist": {"folder": "01_economist", "pattern": r"economist.*\.(epub|pdf)", "topic": "world_affairs"},
    "wired": {"folder": "05_wired", "pattern": r"wired.*\.(epub|pdf)", "topic": "technology"},
    "atlantic": {"folder": "04_atlantic", "pattern": r"atlantic.*\.(epub|pdf)", "topic": "world_affairs"}
}
...
def download_and_process_magazine(magazine_name, magazine_info):
    ...
    for content in contents:
        if content.type == "file" and re.match(pattern, content.name.lower()):
            # 从文件名中提取日期作为唯一标识符
