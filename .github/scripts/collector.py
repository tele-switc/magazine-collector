...
# 【最终修复】使用更宽松、更强大的正则表达式
MAGAZINES = {
    "economist": {"folder": "01_economist", "pattern": r"(economist|the economist).*\d{4}.*\.epub", "topic": "world_affairs"},
    "wired": {"folder": "05_wired", "pattern": r"wired.*\d{4}.*\.epub", "topic": "technology"},
    "atlantic": {"folder": "04_atlantic", "pattern": r"atlantic.*\d{4}.*\.epub", "topic": "world_affairs"}
}
...
def process_all_magazines():
    ...
    for magazine_name, info in MAGAZINES.items():
        ...
        for file_path in source_folder.glob('*.epub'):
            if re.search(pattern, file_path.name.lower()) and file_path.stem not in magazine_contents:
...
def generate_website():
    ...
    <style>
        ...
        :root { --bg-color: #02040a; --text-color: #e6f1ff; --secondary-text: #8892b0;
                ...
                --border-color: rgba(255, 255, 255, 0.1); --glow-color: rgba(0, 191, 255, 0.7); }
        ...
        .card:before { content: ''; position: absolute; inset: -1px; z-index: -1;
                       background: conic-gradient(from var(--angle), transparent 40%, var(--glow-color), transparent);
                       border-radius: inherit; animation: rotate 6s linear infinite; opacity: 0; transition: opacity 0.4s ease; }
